...
| — | **NỢ THỨ BA BỊ BÁC 21/09 — `videodl.log` 644 "rò liên đội"** | — | Kết luận cũ suy từ quyền **TỆP** mà không ai kiểm quyền **THƯ MỤC CHA**. Đo trên mini: `~/Library/Logs` **700**, `~/Library` **700**, `~` **750**; ACL duy nhất là `group:everyone deny delete` — **mặc định macOS, không cấp quyền đọc cho ai**. Muốn mở tệp phải đi QUA thư mục cha ⇒ 644 **không** làm ai đọc được. ⚠ Nhưng đừng ghi thành "an toàn": `dsmemberutil checkmembership -U autotest -G admin` → **là admin** (đối chứng: `nobi_auto` cũng admin) ⇒ `sudo` đọc được bất kể quyền. **Hai chiều cùng dẫn tới: `chmod` và đổi đường log đều KHÔNG đổi một bit rủi ro nào** — một PR như thế chỉ làm người sau tưởng đã xong. Chữa thật = lọc nội dung nhạy cảm **tại nguồn** trước khi chạm stderr, hoặc bàn với chủ máy việc `autotest` có cần admin — cả hai là quyết định **user/chủ máy**, ngoài địa phận lane | — |
