...
| D16 | Tên cụm khi duyệt = "<insight gốc> <kiểu>" (user chốt 23/09 15:58). CHỈ khi hai kiểu trong CÙNG lượt trùng TÊN CUỐI (casefold+strip, tên sau khi đã ghép nhóm) mới chèn nhóm: "<insight gốc> <nhóm> <kiểu>"; tên vừa ghép trùng tên cuối của kiểu thứ ba ⇒ kiểu đó cũng ghép, lặp tới khi hết (có trần). Va chạm còn lại khi mọi kiểu dính vào đã ghép nhóm: CÙNG một kiểu (cùng nhóm + cùng kiểu theo casefold, biến thể hoa/thường) ⇒ giữ nguyên, lúc duyệt tự gộp một cụm; hai kiểu KHÁC nhau ⇒ kiểu đứng sau (thu_tu, id) thêm hậu tố số " 2", " 3"… nhỏ nhất chưa có trong lượt, lặp có trần, vượt trần ⇒ ValueError, không gộp. Không bao giờ hỏi gộp vào cụm vừa tạo trong cùng lượt duyệt | QUYẾT ĐP 18:51 (ĐP-21), user bác được; review M1; điều phối 26/09: so tên cuối; ĐP-126: hậu tố số cho kiểu khác nhau |
...
| D19 | Tên cụm (kể cả luật chèn nhóm, so casefold + chuẩn hoá khoảng trắng) tính cho mọi kiểu MỘT lần lúc `ghi_de_xuat` và lưu trong nháp (`ten_cum`). Sau đó CHỈ `doi_ten` tính lại, và `doi_ten` chỉ đổi tên của CHÍNH hàng vừa đổi — va chạm giải trên hàng đó (cùng kiểu ⇒ lấy đúng tên đã lưu; không thì tên trần → ghép nhóm → hậu tố số), mọi hàng khác giữ tên; luật ghép nhóm lúc đề xuất (D16) không đổi; duyệt/gộp/xoá/chuyển chỉ đọc; `hoan_tac` trả lại đúng tên cũ lưu trong nhật ký. Lúc duyệt KIỂM LẠI va chạm với bảng cụm THẬT ⇒ trùng thì trả hỏi gộp như D13, KHÔNG tự đổi tên | QUYẾT ĐP 00:24 25/09 (ĐP-23); review lượt 2 N6; điều phối 26/09: chỉ tính lại khi đổi tên; ĐP-129: đổi tên chỉ đổi hàng vừa đổi |
