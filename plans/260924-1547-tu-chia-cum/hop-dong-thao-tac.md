...
**Tên cụm lúc duyệt (chính sách chốt sau review, thay "chưa quyết" ở bản trước):** mặc định `"<insight gốc> <kiểu>"`, KHÔNG đổi. CHỈ khi ≥2 kiểu trong CÙNG lượt trùng tên kiểu đã chuẩn hoá (`chuan_hoa_chu` + casefold — khoá `models_cum._khoa_ten`), các kiểu trùng đó (và CHỈ chúng) mới ghép thêm `nhom`: `"<insight gốc> <nhom> <kiểu>"`. Mục đích: một `duyet_het` không bao giờ tự hỏi "gộp vào cụm tôi vừa tạo trong CHÍNH lượt gọi này" — D13 chỉ để bắt trùng với cụm THẬT có từ trước.

**D19 (review lượt 2):** quyết định ghép-nhóm-hay-không CHỐT một lần ngay lúc `ghi_de_xuat` (và mỗi lần `doi_ten`/`gop`/`xoa_kieu` đổi tập nhóm/kiểu), lưu ở cột `ten_cum` trên `cum_nhap` — duyệt CHỈ ĐỌC cột này, không tự tính lại. Trước đây tính lại NGAY LÚC DUYỆT làm tên trôi theo thứ tự duyệt (duyệt từng kiểu một qua nhiều lần gọi riêng lẻ, xoá dần các hàng "đang trùng"). Tái kiểm va chạm với `cum` THẬT vẫn diễn ra Ở LÚC DUYỆT (không phải lúc `ghi_de_xuat`) — một cụm thật trùng tên xuất hiện SAU khi đề xuất đã ghi vẫn trả `trung_cum_co_san` như bình thường, KHÔNG bao giờ tự đổi tên để né va chạm.
