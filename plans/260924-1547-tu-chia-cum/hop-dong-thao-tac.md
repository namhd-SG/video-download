...
**Tên cụm lúc duyệt (D16):** mặc định `"<insight gốc> <kiểu>"`. Khi ≥2 kiểu trong CÙNG lượt có cùng TÊN CUỐI (tên sau khi đã ghép nhóm, nếu có; so bằng `chuan_hoa_chu` + casefold — khoá `models_cum._khoa_ten`), mọi kiểu trong nhóm trùng đó ghép thêm `nhom`: `"<insight gốc> <nhom> <kiểu>"`. Tên vừa ghép có thể lại trùng tên cuối của một kiểu khác (vd `Vest`/`couple` ⇒ "Vest couple", trùng kiểu đơn "Vest couple" của nhóm `khác`) ⇒ kiểu đó cũng ghép nhóm ("khác Vest couple"); lặp tới khi mọi nhóm trùng còn lại chỉ gồm kiểu ĐÃ ghép nhóm (mỗi vòng ghép thêm ≥1 kiểu, kiểu đã ghép không đổi nữa ⇒ tối đa số-kiểu + 1 vòng). **Va chạm còn lại** (mọi kiểu dính vào đã ghép nhóm mà tên cuối vẫn trùng) chia hai ca: (a) CÙNG một kiểu — cùng `nhom` VÀ cùng `kieu` theo casefold, tức biến thể hoa/thường trong cùng nhóm (`Vest`/`couple` và `Vest`/`Couple`) — giữ nguyên, lúc duyệt TỰ GỘP vào MỘT cụm, không hỏi; (b) hai kiểu KHÁC nhau mà tên ghép trùng nhau (`A`/`B couple` và `A B`/`couple` cùng ra "A B couple") — KHÔNG gộp, KHÔNG hỏi: kiểu đứng đầu (theo `thu_tu`, `id`) giữ tên, mỗi kiểu khác trong nhóm trùng nhận hậu tố số `" 2"`, `" 3"`… — số nhỏ nhất (từ 2) mà tên đó chưa có trong lượt, kể cả một kiểu có sẵn tên như thế (vd kiểu đơn "A B couple 2" ⇒ hậu tố " 3"); lặp lại tới khi hết trùng, có trần (số kiểu + 1 vòng); vượt trần ⇒ lỗi `ValueError` (400), không bao giờ gộp thay. Luật "ghép nhóm CẢ HAI hàng" và hậu tố theo `thu_tu` ở mục này CHỈ chạy lúc `ghi_de_xuat`; `doi_ten` có luật riêng (mục D19). Mục đích: `duyet_het` không bao giờ tự hỏi "gộp vào cụm tôi vừa tạo trong CHÍNH lượt gọi này" — D13 chỉ để bắt trùng với cụm THẬT có từ trước.

**D19 — khi nào tên cụm được tính:** `ten_cum` (cột trên `cum_nhap`, kèm luật ghép nhóm ở trên) được tính cho MỌI kiểu đúng MỘT lần lúc `ghi_de_xuat`. Sau đó nó CHỈ được tính lại khi `doi_ten`, và `doi_ten` CHỈ đổi `ten_cum` của ĐÚNG hàng vừa đổi tên (X); mọi hàng khác giữ nguyên tên đã lưu, bất kể `thu_tu` — không lan tiền tố nhóm, không hậu tố sang hàng khác. Va chạm được giải trên CHÍNH X, so với tên đã lưu của mọi hàng khác: có hàng CÙNG một kiểu với X (cùng `nhom` + `kieu` theo casefold) ⇒ X lấy đúng tên đã lưu của hàng đó (để lúc duyệt tự gộp, D16); không thì tên trần, trùng ⇒ tên ghép nhóm, vẫn trùng ⇒ tên ghép + hậu tố `" N"` nhỏ nhất (từ 2) chưa có trong lượt (có trần; vượt ⇒ 400). Luật ghép nhóm lúc đề xuất (D16) không đổi. Duyệt (`duyet_kieu`/`duyet_het`), `gop`, `xoa_kieu`, `chuyen`, `ngoai_chu_de`, `tra_ve` CHỈ ĐỌC `ten_cum`, không tính lại tên hàng nào — kể cả khi thao tác đó làm mất "anh em" trùng tên của một kiểu (kiểu còn lại giữ tiền tố nhóm). `hoan_tac` trả lại ĐÚNG `ten_cum` cũ đã lưu trong dòng nhật ký gốc (`doi_ten`: `ten_cum_truoc` của hàng vừa đổi; `gop`: `tu_ten_cum`; `xoa_kieu`: `ten_cum`), không tính lại. `GET /chia/{job_id}` trả `ten_cum` cho từng kiểu. Lý do: tính lại trên tập hàng CÒN SỐNG làm tên trôi theo thứ tự thao tác (duyệt riêng "Vest couple" rồi đổi tên một kiểu không liên quan từng biến "Đồng phục couple" thành "couple" trần). Tái kiểm va chạm với `cum` THẬT vẫn diễn ra Ở LÚC DUYỆT — một cụm thật trùng tên xuất hiện SAU khi đề xuất đã ghi vẫn trả `trung_cum_co_san` như bình thường, KHÔNG bao giờ tự đổi tên để né va chạm.
